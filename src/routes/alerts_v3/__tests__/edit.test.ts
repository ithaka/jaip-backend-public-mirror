...
import {
  ...
  full_targeted_alert_with_groups,
...
test(`requests the ${route} route with valid alert but no id`, async () => {
  const res = await app.inject({
    method: "PATCH",
    url: `${route}`,
...
test(`requests the ${route} route with valid alert and no permissions`, async () => {
  ...
  const res = await app.inject({
    method: "PATCH",
    url: `${route}`,
    payload: full_targeted_alert_with_facilities,
  });
...
test(`requests the ${route} route with valid alert but invalid target and edit facility permissions`, async () => {
  ...
  const res = await app.inject({
    method: "PATCH",
    url: `${route}`,
    payload: full_targeted_alert_with_groups,
    headers: {
...
test(`requests the ${route} route with valid body and edit facility permissions`, async () => {
  ...
  axios.post = jest
    ...
    .mockReturnValue(axios_session_data_with_email) as typeof axios.post;
  db_mock.get_first_user.mockResolvedValueOnce(basic_admin);
  db_mock.update_targeted_alert.mockResolvedValueOnce([
    full_targeted_alert,
    null,
  ]);
  ...
  expect(db_mock.update_targeted_alert).toHaveBeenCalledTimes(1);
