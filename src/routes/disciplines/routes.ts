...
async function routes(fastify: FastifyInstance, opts: RouteShorthandOptions) {
  ...
  // NOTE: This should be handled by the schema, but it's not registering with typescript
  interface discParams {
    code: string;
  }

  opts.schema = route_schemas.disciplines;
  fastify.get<{ Params: discParams }>("/disciplines", opts, handler);

  opts.schema = route_schemas.journals;
  fastify.get<{ Params: discParams }>("/disciplines/:code", opts, handler);
}
