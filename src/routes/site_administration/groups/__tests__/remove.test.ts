...
import {
  ...
  basic_user_ungrouped_remove_groups,
  delete_group_body_invalid,
  delete_group_body_valid,
...
const route = `${prefix}${get_route(route_schemas.delete_group)}`;
...
test(`requests the ${route} route with invalid body`, async () => {
  const res = await app.inject({
    method: "DELETE",
    url: `${route}`,
    payload: delete_group_body_invalid,
  });
  expect(res.statusCode).toEqual(400);
});
...
test(`requests the ${route} route with valid body and no subdomain permissions`, async () => {
  ...
  const res = await app.inject({
    method: "DELETE",
    url: `${route}`,
    payload: delete_group_body_valid,
  });
...
test(`requests the ${route} route with valid body and remove subdomain permissions`, async () => {
  ...
  db_mock.remove_group.mockResolvedValueOnce(null);
  ...
  expect(db_mock.remove_group).toHaveBeenCalledTimes(1);
